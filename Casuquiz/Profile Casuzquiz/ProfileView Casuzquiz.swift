...
struct ProfileViewCasuzquiz: View {
    ...
    var body: some View {
        VStack {
            ...
            .sheet(isPresented: $showImagePickerBB) {
                ImagePickerViewCasuzquiz(showImagePicker: self.$showImagePickerBB) { image in
                    if let image = image {
...
#Preview {
    ContentViewCasuzquiz(showLoading: false, selectedTab: .profile)
}
